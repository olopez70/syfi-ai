# Fix failing test: main.py execution test due to ProfileBuilder initialization error
...
1. **Configuration parsing error:**
   ```
   Parsing failed: 'Configuration' object has no attribute 'get'
   ```

2. **ProfileBuilder initialization error:**
   ```
   Error: ProfileBuilder.__init__() missing 1 required positional argument: 'db_manager'
   ```
...
- [ ] `test_main_execution` passes
- [ ] `main.py` runs without errors
- [ ] Configuration parsing works correctly
- [ ] ProfileBuilder initializes with correct parameters
- [ ] All existing tests continue to pass
- [ ] No regression in web interface functionality
